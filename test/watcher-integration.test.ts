...
import { watchPr, defaultHeal } from "../src/watcher/watcher.ts";
...
test("built-in pr-ci heals a red 'test' PR end to end via the watcher", async () => {
  ...
  const shell = async (cmd: string, args: string[]): Promise<ShellResult> => {
    if (cmd === "gh" && args[0] === "pr" && args[1] === "checks") {
      const rows = repaired ? [{ name: "unit-tests", bucket: "pass" }] : [{ name: "unit-tests", bucket: "fail" }];
      return { stdout: JSON.stringify(rows), stderr: "", code: repaired ? 0 : 8 };
    }
    if (cmd === "bash") { repaired = true; return { stdout: "", stderr: "", code: 0 }; } // lint-fix ran
    return { stdout: "", stderr: "", code: 0 };
  };

  const gh = new GhClient(shell);
  const heal = defaultHeal({ domain, shell, smallModel: new FakeSmallModel([]), playbook: pb, repo: "r", pr: 5 });
  const res = await watchPr({ repo: "r", pr: 5, pollMs: 0 }, { gh, heal, playbook: pb, sleep: async () => {} });
